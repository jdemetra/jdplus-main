package jdplus.x13.desktop.plugin;
...
public class X13RuntimeDependenciesTest {

    @Test
    public void test() throws IOException {
        assertThat(getRuntimeDependencies(X13RuntimeDependenciesTest.class))
                .describedAs("Check runtime dependencies")
                .satisfies(X13RuntimeDependenciesTest::checkX13)
                .hasSize(5);
    }

    private static void checkX13(List<? extends GAV> coordinates) {
        assertThatGroupId(coordinates, "eu.europa.ec.joinup.sat")
                .has(sameVersion())
                .extracting(GAV::getArtifactId)
                .are(matchingPattern(compile("^jdplus-x13-base-\\w+$")))
                .hasSize(5);
    }
