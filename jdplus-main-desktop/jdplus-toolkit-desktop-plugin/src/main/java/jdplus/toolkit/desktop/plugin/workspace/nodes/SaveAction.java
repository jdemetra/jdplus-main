...
public final class SaveAction extends SingleNodeAction<ItemWsNode> {
    ...
    protected void performAction(ItemWsNode context) {
        ...
        if (cur != null && !cur.isReadOnly()) {
            NotifyDescriptor nd = new NotifyDescriptor.Confirmation(SAVE_MESSAGE, NotifyDescriptor.OK_CANCEL_OPTION);
            if (DialogDisplayer.getDefault().notify(nd) != NotifyDescriptor.OK_OPTION) {
                return;
            }
            if (cur.save(DemetraVersion.JD3)) {
                WorkspaceFactory.Event ev = new WorkspaceFactory.Event(cur.getOwner(), cur.getId(), WorkspaceFactory.Event.SAVE);
