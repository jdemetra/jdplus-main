...
import jdplus.toolkit.desktop.plugin.util.ShowInFolderActionListener;
...
public class FileRepository extends AbstractWorkspaceRepository implements LookupListener {
    ...
    public boolean save(Workspace ws, DemetraVersion version, boolean force) {
        if (super.save(ws, version, force)) {
            notifySuccess(decode(ws.getDataSource()));
            return true;
        }
        return false;
    }
    ...
    public boolean saveAs(Workspace ws, DemetraVersion version) {
        wsFileChooser.setSelectionApprover(SingleFileExporter.overwriteApprover());
        File file = wsFileChooser.showSaveDialog();
        wsFileChooser.setSelectionApprover(null);
        if (file == null) {
            return false;
        }
        ...
        try {
            ...
            notifyError(ws, "Failed to save workspace to file '" + file + "'", ex);
            return false;
        }
    }
    ...
    public boolean load(Workspace ws) {
        ...
        try (FileWorkspace storage = FileWorkspace.open(file.toPath())) {
            ...
            notifyError(ws, "Failed to load workspace from file '" + file + "'", ex);
            return false;
        }
...
//            target.set(name, cal);
    ...
    private static void notifySuccess(File file) {
        NotifyUtil.show("Workspace saved", "Show in folder", MessageType.SUCCESS, ShowInFolderActionListener.of(file), null, null);
    }

    private static void notifyError(Workspace ws, String message, Exception ex) {
        NotifyUtil.show("Workspace error", message, MessageType.ERROR,
                event -> showException("workspace " + ws.getName(), unwrapException(ex)), null, null);
    }
}
