...
public class HtmlLogLevelDetails implements HtmlLogFormatter<LogLevelModule.Info> {
    ...
    public void write(HtmlStream stream, LogLevelModule.Info details, boolean verbose) throws IOException {
        stream.write("Modified sum of squares on logs: ").write(new Formatter(Locale.ROOT).format("%6g", details.getLogs()).toString());
        stream.newLine();
        stream.write("Modified sum of squares on levels: ").write(new Formatter(Locale.ROOT).format("%6g", details.getLevels()).toString());
        stream.newLine();
