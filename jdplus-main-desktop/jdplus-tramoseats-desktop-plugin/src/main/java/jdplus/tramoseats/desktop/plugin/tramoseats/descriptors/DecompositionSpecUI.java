...
import jdplus.tramoseats.base.api.seats.DecompositionSpec.BiasCorrection;
...
public class DecompositionSpecUI extends BaseTramoSeatsSpecUI {
    ...
    public boolean isBias() {
        return seats().getBiasCorrection() != BiasCorrection.None;
    }

    public void setBias(boolean value) {
        update(seats().toBuilder().biasCorrection(value ? BiasCorrection.Legacy : BiasCorrection.None).build());
    }
    ...
    private static final int RMOD_ID = 0, EPSPHI_ID = 1, SMOD_ID = 2, SMOD1_ID = 3, NOADMISS_ID = 4, XL_ID = 5, WK_ID = 6, NFCASTS_ID = 10, NBCASTS_ID = 11, BIAS_ID = 12;
    ...
    @NbBundle.Messages({
        "decompositionSpecUI.biasDesc.name=Bias correction",
        "decompositionSpecUI.biasDesc.desc=Correction for the bias generated by the log transformation (if any)"
    })
    private EnhancedPropertyDescriptor biasDesc() {
        if (tramo().getTransform().getFunction() == TransformationType.None)
            return null;
        try {
            ...
            edesc.setRefreshMode(EnhancedPropertyDescriptor.Refresh.All);
            desc.setDisplayName(Bundle.decompositionSpecUI_biasDesc_name());
            desc.setShortDescription(Bundle.decompositionSpecUI_biasDesc_desc());
            edesc.setReadOnly(isRo());
            return edesc;
        } catch (IntrospectionException ex) {
            return null;
