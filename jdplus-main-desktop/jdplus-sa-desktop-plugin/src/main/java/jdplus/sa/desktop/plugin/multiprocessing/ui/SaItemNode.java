...
public class SaItemNode extends AbstractNode {
    ...
    private Image lookupIcon(int type, boolean opened) {
        ...
        return DataSourceManager.get()
                .getImage(item.getDefinition().getTs().getMoniker(), type, opened);
    }
    ...
    private static Sheet.Set getDataSheetSet(SaItem item, NodePropertySetBuilder b) {
        ...
        if (!data.isEmpty()) {
            b.withEnum(RegularFrequency.class)
                    .select("frequency", () -> RegularFrequency.parseTsUnit(data.getTsUnit()), null)
                    .display("Frequency")
                    .add();
            b.with(String.class)
                    .select(
                            "startPeriod",
                            () -> data.getDomain().getStartPeriod().getStartAsShortString(),
                            null)
            ...
            b.with(String.class)
                    .select(
                            "lastPeriod",
                            () -> data.getDomain().getLastPeriod().getStartAsShortString(),
                            null)
                    .display("Last period")
                    .add();
            ...
            b.with(String.class)
                    .selectConst("InvalidDataCause", data.getEmptyCause())
                    .display("Invalid data cause")
                    .add();
        }
