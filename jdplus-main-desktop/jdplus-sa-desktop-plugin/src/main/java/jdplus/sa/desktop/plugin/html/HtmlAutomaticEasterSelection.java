...
public class HtmlAutomaticEasterSelection implements HtmlLogFormatter<EasterDetectionModule.Info> {
    ...
    @Override
    public Class<EasterDetectionModule.Info> getSourceClass() {
        return EasterDetectionModule.Info.class;
    }

    @Override
    public void write(HtmlStream stream, EasterDetectionModule.Info details, boolean verbose) throws IOException {
        ...
        LikelihoodStatistics ll0 = details.getLl0();
        stream.open(HtmlTag.TABLEROW);
        if (details.getBest() < 0) {
            stream.write(new HtmlTableCell("no easter", HtmlStyle.Bold).withWidth(100));
            stream.write(new HtmlTableCell(DF4.format(ll0.getAIC()), HtmlStyle.Bold).withWidth(100));
            stream.write(new HtmlTableCell(DF4.format(ll0.getBIC()), HtmlStyle.Bold).withWidth(100));
        } else {
            stream.write(new HtmlTableCell("no easter").withWidth(100));
            stream.write(new HtmlTableCell(DF4.format(ll0.getAIC())).withWidth(100));
            stream.write(new HtmlTableCell(DF4.format(ll0.getBIC())).withWidth(100));
        }
        stream.close(HtmlTag.TABLEROW);
