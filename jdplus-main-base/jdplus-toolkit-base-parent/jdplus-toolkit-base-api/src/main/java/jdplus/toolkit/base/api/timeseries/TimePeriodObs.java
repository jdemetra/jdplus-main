...
public class TimePeriodObs implements TimeSeriesObs<TimePeriod>, HasShortStringRepresentation {
    ...
    @StaticFactoryMethod
    public static @NonNull TimePeriodObs parse(@NonNull CharSequence text) {
        int index = text.toString().indexOf("=");
        if (index < 0) {
            throw new IllegalArgumentException("Invalid CalendarPeriodObs text: " + text);
        }
        return of(
                TimePeriod.parse(text.subSequence(0, index)),
